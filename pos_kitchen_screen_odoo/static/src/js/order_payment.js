...
patch(Order.prototype, {
    ...
    async pay() {
        ...
        // Check kitchen order status to ensure all items are ready
        await this.orm.call("pos.order", "check_order", ["", order_name]).then(function (result) {
            if (result.category) {
                ...
                self.popup.add(ErrorPopup, {
                    ...
                    body: _t("Please complete all the food preparation before proceeding with payment."),
                });
            } else {
                self.kitchen = true;
            }
        ...
        // Ensure all items in the kitchen order are marked as "ready" before proceeding
        var isAllReady = this.orderlines.every(line => line.order_status == 'ready');
        if (!isAllReady) {
            this.kitchen = false;
            this.popup.add(ErrorPopup, {
                title: _t("Food is not ready"),
                body: _t("All items must be marked as 'Ready' before proceeding to payment."),
            });
            return;
        }
