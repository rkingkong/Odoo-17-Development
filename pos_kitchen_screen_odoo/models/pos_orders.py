...
class PosOrder(models.Model):
    ...
    # Modified write method to handle creating a new kitchen order for "ready" or "completed" orders.
    def write(self, vals):
        ...
        for order in self:
            # Check if the order is in "ready" or "completed" status
            if order.order_status in ["ready", "completed"]:
                # Create a new order with updated values for additional items
                new_vals = vals.copy()
                new_vals['order_status'] = 'draft'
    ...
    # Modified create method to ensure new tickets are created for "ready" or "completed" orders.
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if pos_orders:
                # If there is an existing order and it is marked as "ready" or "completed"
                if pos_orders.order_status in ["ready", "completed"]:
                    # Create a new order instead of modifying the existing one
                    vals['order_status'] = 'draft'  # Set new order status to draft
                    vals['is_cooking'] = True  # Set cooking to true for new orders
        ...
        # Fallback to create a new order
